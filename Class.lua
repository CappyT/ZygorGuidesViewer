...
function Class:New(classType)
	...
	if STRICT_CLASSES then setmetatable(class,{__index=function(tab,ind) if not rawget(tab,"__UNSTRICT_CLASS") and type(ind)=="string" and not allowed_nils[classType.."."..ind] and ind:sub(1,1):match("[A-Z]") then error(("|c00cc00%s|r.|c00ff00%s|r not found"):format(classType,ind),2) end end}) end
